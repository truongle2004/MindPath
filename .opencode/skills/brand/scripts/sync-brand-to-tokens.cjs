...
function adjustBrightness(hex, percent) {
  ...
  const g = Math.min(255, Math.max(0, ((num >> 8) & 0x00_ff) + Math.round(255 * percent)));
  const b = Math.min(255, Math.max(0, (num & 0x00_00_ff) + Math.round(255 * percent)));
...
function main() {
  ...
  if (fs.existsSync(generateScript)) {
    try {
      ...
      console.error('⚠️  Failed to regenerate CSS:', error.message);
    }
  }
